    ...
    def handle_starttag(self, tag, attrs):
        ...
        if self.in_article:
            ...
            if tag in self.LEVEL_ELEMENTS:
                ...
                print(
                    "{}{} ".format(
                        " " * self.current_indent * self.SPACES_PER_LEVEL, char
                    ),
                    end="",
                )
        ...
        if self.in_article:
            if tag in self.LEVEL_ELEMENTS:
                ...
                if href:
                    fprint(
                        " ({}{}{}{})".format(
                            TermFmt.UNDERLINE, TermFmt.BLUE, href, TermFmt.RESET
                        ),
                        end="",
                    )
...
def main():
    ...
    arg_parser.add_argument(
        "-n",
        "--no-formatting",
        help="Disable formatting, not printing anything \
other than the plain page contents. This option omits the page title.",
        action="store_false",
    )
    arg_parser.add_argument(
        "-d", "--dch", help="Format for a Debian changelog.", action="store_true"
    )
    arg_parser.add_argument(
        "page",
        help='The docs page to print. This may be a path relative to \
